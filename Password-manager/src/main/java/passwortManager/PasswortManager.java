...
public class PasswortManager {
	...
	private static final String PATH_TO_DB = "src/main/resources/database.db";
	...
	private static void erweiterteOptionen() {
		...
		switch (nr) {
		case 0:
			Delete.deleteAll(PATH_TO_DB);
			break;
		case 1:
			masterPasswort = passwordInput.askPassword();
			XmlParser.export(masterPasswort, PATH_TO_DB);
			break;
		case 2:
			masterPasswort = passwordInput.askPassword();
			XmlParser.inport(masterPasswort, PATH_TO_DB);
			break;
		default:
			p(UNK);
		}
	...
	private static void readMailData() {
		...
		String[][] mailData = Read.getMailData(masterPasswort, PATH_TO_DB);
	...
	private static void update() {
		...
		case 0:
			...
			switch (snr) {
			case 0:
				Update.update(ENTITETEN[snr], pid, newEntry, null, PATH_TO_DB);
				break;
			case 1:
				masterPasswort = passwordInput.askPassword();
				Update.update(ENTITETEN[snr], pid, newEntry, masterPasswort, PATH_TO_DB);
				break;
			case 2:
				Update.updateMail(pid, newEntry, PATH_TO_DB);
				break;
			case 3:
				Update.updateWebsite(pid, newEntry, PATH_TO_DB);
			}
			break;
		case 1:
			...
			masterPasswort = passwordInput.askPassword();
			Update.updateMailPasswort(mid, newPasswort, masterPasswort, PATH_TO_DB);
			break;
		}
		p("data updated");
	}
	...
	private static void write() {
		...
		case 0:
			...
			masterPasswort = passwordInput.askPassword();
			Insert.write(username, passwort, mail, web, masterPasswort, PATH_TO_DB);
			break;
		case 1:
			String m_mail = faq("mail"), m_passwort = faq("password"), m_masterPasswort = passwordInput.askPassword();
			Insert.addNewMail(m_mail, m_passwort, m_masterPasswort, PATH_TO_DB);
			break;
		default:
			p(UNK);
			write();
	...
	private static void read() {
		...
		String[] webPageNames = Read.readKeysWeb(PATH_TO_DB);
		...
		String[] data = Read.getData(nr + 1, masterPasswort, PATH_TO_DB);
		for (int k = 0; k < 4; k++) {
			p(NAMES[k] + ":" + data[k]);
		}
	}
